...
#include "usercode.h"
#include "usercode_wallfollow.h"    /* 沿右岸自主主循环（编译宏 WALL_FOLLOW_MODE 切换） */
...
#ifdef WALL_FOLLOW_MODE
    UserLogic_WallFollow();	/* 沿右湖岸自主绕行（Keil Define 加 WALL_FOLLOW_MODE 启用） */
#else
    UserLogic_Code();		/* 默认：树莓派遥控 + JY901 PID（队友的原版） */
#endif
}
